...
def _build_activation_link(token: str) -> str:
    return f"{client_base_url()}/activate-account?token={quote(token)}"


def send_activation_email(client: "ClientAccount", token: str) -> bool:
    """Send an account activation email with a magic link."""
    ...
    text = (
        ...
        f"Thanks for booking with {brand}. Use the link below to finish creating your portal account "
                    ...
                    <table role="presentation" style="max-width:600px;width:100%;background-color:#ffffff;border-radius:12px;box-shadow:0 2px 8px rgba(0,0,0,0.1);">
                        <tr>
                            <td style="padding:40px 40px 30px;text-align:center;border-bottom:1px solid #e5e5e5;">
                                ...
                                <p style="margin:8px 0 0;font-size:14px;color:#666;">Activate your portal access</p>
                            </td>
                        </tr>
                        <tr>
                            <td style="padding:40px;">
                                <p style="margin:0 0 20px;font-size:16px;line-height:24px;color:#333;">
                                    Hi {escape(client.display_name or 'guest')},
                                </p>
                                <p style="margin:0 0 24px;font-size:16px;line-height:24px;color:#333;">
                                    Thanks for booking with {escape(brand)}. Finish creating your portal account so you can manage bookings and inspiration.
                                </p>
                                <div style="margin:0 0 28px;text-align:center;">
                                    <a href="{link}" style="display:inline-block;padding:14px 32px;background-color:#111;color:#ffffff;text-decoration:none;border-radius:6px;font-weight:600;font-size:15px;">
                                        Activate my account
                                    </a>
                                </div>
                                <p style="margin:0 0 18px;font-size:14px;line-height:21px;color:#555;">
