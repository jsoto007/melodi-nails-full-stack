...
def send_signup_email(client: "ClientAccount", verification_code: str | None = None) -> bool:
    """Send the welcome email and optional verification code to a new client."""
    ...
    verify_hint = (
        ...
        f"Verify online: {client_base_url()}/verify-email?email={quote(client.email)}&code={quote(verification_code)}\n\n"
        if verification_code
        else ""
    )
    text = (
        f"Hi {client.display_name or 'there'},\n\n"
        f"Thanks for creating a portal account with {brand}. "
    ...
    verify_cta = ""
    if verification_code:
        verify_url = f"{client_base_url()}/verify-email?email={quote(client.email)}&code={quote(verification_code)}"
        verify_cta = f"""
            <p style="margin:0 0 12px;font-size:14px;font-weight:600;color:#666;">
                Your verification code:
            </p>
            <div style="margin:0 0 24px;text-align:center;">
                <div style="display:inline-block;padding:16px 32px;background-color:#0b1f4d;color:#ffffff;font-size:24px;font-weight:700;letter-spacing:6px;border-radius:8px;font-family:'Courier New',monospace;">
                    {escape(verification_code)}
                </div>
            </div>
            <div style="margin:0 0 24px;text-align:center;">
                    ...
                    <table role="presentation" style="max-width:600px;width:100%;background-color:#ffffff;border-radius:12px;box-shadow:0 2px 8px rgba(0,0,0,0.1);">
                        <tr>
                            <td style="padding:40px 40px 30px;text-align:center;border-bottom:1px solid #e5e5e5;">
                                ...
                                <p style="margin:8px 0 0;font-size:14px;color:#666;">Welcome to your portal</p>
                            </td>
                        </tr>
                        <tr>
                            <td style="padding:40px;">
                                ...
                                <p style="margin:0 0 24px;font-size:16px;line-height:24px;color:#333;">
                                    Thanks for creating a portal account with {escape(brand)}. You can manage your bookings, documents, and inspiration online.
                                </p>
                                {verify_cta}
